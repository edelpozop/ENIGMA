...
int main(int argc, char* argv[]) {
    ...
    if (argc < 2) {
        XBT_CRITICAL("Usage: %s <platform_file.xml> [num_messages=1000] [num_fog_servers=4] [num_edge_devices=100] [packet_size_bytes=4096] [options]", 
                     argv[0]);
        ...
        XBT_CRITICAL("  num_edge_devices: Number of edge devices (IoTs) to use (default: 100)");
    ...
    int num_edge_devices = (argc > 4) ? std::atoi(argv[4]) : 100;
    int packet_size = (argc > 5) ? std::atoi(argv[5]) : 4096; // bytes per message
    ...
    // Classify hosts by name pattern
    std::vector<sg4::Host*> edge_hosts, fog_hosts;
    
    for (auto* host : hosts) {
        std::string name = host->get_cname();
        // Check if it matches edge_N pattern
        if (name.substr(0, 5) == "edge_") {
            edge_hosts.push_back(host);
        } 
        // Check if it matches fog_N pattern
        else if (name.substr(0, 4) == "fog_") {
            fog_hosts.push_back(host);
        }
    }

    // Validate num_edge_devices
    if (num_edge_devices <= 0) {
        XBT_WARN("num_edge_devices must be > 0, using default: 100");
        num_edge_devices = 100;
    }
    
    if (num_edge_devices > static_cast<int>(edge_hosts.size())) {
        XBT_WARN("num_edge_devices (%d) cannot be greater than available edge hosts (%zu), using all available (%zu)",
                 num_edge_devices, edge_hosts.size(), edge_hosts.size());
        num_edge_devices = edge_hosts.size();
    }
    ...
    XBT_INFO("  Number of edge devices (IoTs) to use: %d", num_edge_devices);
    ...
    for (int i = 0; i < num_fog_servers; i++) {
        fog_hosts[i]->add_actor("fog_server_" + std::to_string(i),
                                FogServer(i, num_edge_devices));
    }
    
    // Deploy Edge devices (only the requested number)
    for (int i = 0; i < num_edge_devices; i++) {
        edge_hosts[i]->add_actor("edge_device_" + std::to_string(i),
                                 EdgeDevice(i, num_messages, num_fog_servers, static_cast<size_t>(packet_size)));
    }
